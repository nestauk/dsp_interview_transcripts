...
def main(selection="eom", min_length=5, min_cluster_size=50, reduction_strategy="embeddings"):
    ...
    data = pd.read_csv(f"{PROJECT_DIR}/data/bit_france/converted/combined_transcripts_with_roles.csv")
    logger.info(f"N records: {len(data)}")
    ...
    embeddings_path = (
        f"{PROJECT_DIR}/dsp_interview_transcripts/pipeline/bit_france/outputs/embeddings_min_length_{min_length}.npy"
    )
    ...
    output_path = f"{PROJECT_DIR}/dsp_interview_transcripts/pipeline/bit_france/outputs/bertopic_visualization_selection_{selection}_min_length_{min_length}_min_cluster_{min_cluster_size}_red_{reduction_strategy}.html"
    fig.write_html(output_path)

    # Default BERTopic summary info
    topic_info = topic_model.get_topic_info()
    topic_info_path = f"{PROJECT_DIR}/dsp_interview_transcripts/pipeline/bit_france/outputs/bertopic_topic_info_selection_{selection}_min_length_{min_length}_min_cluster_{min_cluster_size}_red_{reduction_strategy}.csv"
    ...
    noise_path = f"{PROJECT_DIR}/dsp_interview_transcripts/pipeline/bit_france/outputs/noise_prop_selection_{selection}_min_length_{min_length}_min_cluster_{min_cluster_size}_red_{reduction_strategy}.txt"
    ...
    data_out_path = f"{PROJECT_DIR}/dsp_interview_transcripts/pipeline/bit_france/outputs/speaker_data_topics_selection_{selection}_min_length_{min_length}_min_cluster_{min_cluster_size}_red_{reduction_strategy}.csv"
    ...
    word_count_path = f"{PROJECT_DIR}/dsp_interview_transcripts/pipeline/bit_france/outputs/topic_median_word_count_selection_{selection}_min_length_{min_length}_min_cluster_{min_cluster_size}_red_{reduction_strategy}.csv"
