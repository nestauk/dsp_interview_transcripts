...
class NameDescription(BaseModel):
    """
    A model representing a named group of documents, including a brief description.

    Attributes:
        name (str): A concise, informative name for this group of documents.
...
def name_topics(
    ...
    Args:
        topic_info (pd.DataFrame): A DataFrame containing information about topics,
            with columns for text samples, top words, and topic labels.
        llm_chain (Any): A language model chain with an `invoke` method that accepts
            a dictionary containing 'question', 'docs', and 'keywords'.
        topics (List[str]): A list of topic identifiers to process.
        text_col (str, optional): The name of the column in `topic_info` containing
            text data for each topic. Defaults to "text_clean".
        top_words_col (str, optional): The name of the column in `topic_info` containing
            the top words for each topic. Defaults to "Top Words".
        topic_label_col (str, optional): The name of the column in `topic_info` that
            indicates topic labels. Defaults to "Cluster".
        question (str, optional): A question to provide context to the language model
            when generating names and descriptions. Defaults to an empty string.

    Returns:
        Dict[str, Dict[str, str]]: A dictionary where each key is a topic identifier
        and each value is a dictionary containing 'name' and 'description' for that topic.

    Example:
        >>> name_topics(topic_info=df, llm_chain=my_llm_chain, topics=["Topic 1", "Topic 2"], question="What is this topic about?")
        {
...
if __name__ == "__main__":
    ...
    for key in [0, 1, 4, 6, 7, 9]:  # these questions are the ones with >100 data points in each
        ...
        results = name_topics(
            df_for_summarisation,
            llm_chain,
            topic_list,
            text_col="Representative_Docs",
            top_words_col="Representation",
            topic_label_col="Topic",
            question=temp_df["question"].unique()[0],
        )

        df_for_summarisation["name"] = df_for_summarisation["Topic"].map(
            lambda x: results[x]["name"]
            if x in results and isinstance(results[x], dict) and "name" in results[x]
            else None
        )
        df_for_summarisation["description"] = df_for_summarisation["Topic"].map(
        ...
        df_vis = df_vis.merge(
            df_for_summarisation[["Topic", "name", "description"]], left_on="Topic", right_on="Topic", how="left"
        )
        full_dfs[key] = df_vis
    ...
    for key in full_dfs.keys():
        ...
        name_count_chart = (
            alt.Chart(df)
            .mark_bar()
            .encode(y=alt.Y("Name:N", title=None), x=alt.X("count()", title="Count"))
            .properties(
                # title='Count of Values of Name'
            )
        )

        sentiment_proportion_chart = (
            alt.Chart(df)
            .mark_bar()
            .encode(
                ...
                color=alt.Color(
                    "sentiment:N",
                    title="Sentiment",
                    scale=alt.Scale(domain=["Negative", "Neutral", "Positive"], range=["red", "gray", "green"]),
                ),
            )
            .properties(
    ...
    with pd.ExcelWriter(excel_file, engine="openpyxl") as writer:
        ...
        for key in summary_dfs.keys():
            ...
            df[
                ["question", "Topic", "Count", "Name", "Representation", "name", "description", "Representative_Docs"]
