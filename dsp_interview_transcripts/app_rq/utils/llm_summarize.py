...
quote_prompt = PromptTemplate.from_template(
    ...
    "Return them as a valid Python list of 5 strings. Do not modify the selected documents in any way. No explanation.\n"
    'Format: ["Doc 1", "Doc 2", "Doc 3", "Doc 4", "Doc 5"]'
)
...
def summarize_and_quote(texts: List[str], question: str) -> Tuple[str, List[str]]:
    ...
    Returns:
        Tuple[str, List[str]]:
            ...
            - A list of 5 direct quotes from the input texts that support the answer.
...
def generate_single_summary(
    ...
    extracted_texts = [txt for sublist in df[text_col] for txt in sublist]
...
def generate_summaries(
    ...
    for rq_id, question in rq_dict.items():
        ...
        long_dfs[question], output["answer"], output["quotes"] = generate_single_summary(
            df, question, rq_id, output_dir, test_mode, text_col=text_col
        )
...
def generate_full_summary_output(
    ...
    per_rq_outputs: Dict[str, Dict[str, object]],
    text_col: str = "text",
) -> pd.DataFrame:
    """
    ...
    for _, question in rq_dict.items():
        merged_output = pd.merge(
            long_dfs[question][[text_col, "identifier", "id"]],
        ...
        full_summary_df = pd.concat(
            [full_summary_df, merged_output[["question", "answer", "id", "identifier", text_col, "quotes"]]],
            ignore_index=True,
        )
