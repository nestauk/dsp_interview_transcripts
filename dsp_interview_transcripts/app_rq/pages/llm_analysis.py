...
def run_analysis(n_clicks, contents, column_info, rq_text, test_mode, session_id):
    ...
    # === TEST MODE: skip LLM ===
    if test_mode:
        output_dir = os.path.join("outputs", "mock_outputs")
        ...
        for rq_id in rq_dict:
            ...
            if mock_path.exists():
                ...
                mock_quotes = sample_df[text_col].tolist()
    ...
    per_rq_outputs, long_dfs = generate_summaries(
        rq_dict, output_paths, output_dir, test_mode
    )  # doesn't call an LLM if we're in test mode
    # Generate a table with answers and quotes for all RQs
    full_summary_df = generate_full_summary_output(rq_dict, long_dfs, per_rq_outputs)
...
def display_results(output_dir, rq_dict):
...
def display_conversation(n_clicks_list, contents, column_info):
    """
    ...
    quote_text = matching_row[text_col].iloc[0]
    ...
    conversation_display = [
        html.Div([html.Strong(f"{convo_df[role_col]}. "), html.Span(highlight_text(row))])
